...
// Call `Call` After `Wrap` to start up a ID.
func Call(co ID, args ...interface{}) error {
	e := findEntity(co)
	...
	return func() (err error) {
		defer func() {
			if v := recover(); v != nil {
				err = fmt.Errorf("Coroutine ID %s called failed: %w", co, v.(error))
			}
		}()
		defer func() {
